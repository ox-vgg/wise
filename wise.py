...
def search_using_text(args):
    ...
    print('TODO: searching images in %s using text query \"%s\"' % (getattr(args,'in'), query))
    return

def search_using_an_image(args):
    print('TODO: searching for images similar to %s in the folder %s' % (getattr(args, 'like'), getattr(args, 'in')))
    return

def search_using_images(args):
    print('TODO: searching folder [%s] for class of objects represented in folder [%s]' % (getattr(args, 'in'), getattr(args, 'like')))
    return

def init(args):
    print('TODO: preparing folder [%s] for search' % (getattr(args, 'in')))
    print('This process involves extracting features, preparing approximate nearest neighbourhood search object, etc. and therefore may take long time to complete.')
    return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--query",
                        type=str,
                        required=False,
                        help="search query (e.g. a black swan)"
                        )
    ...
    if args.command == "search":
        if getattr(args, 'query') or getattr(args, 'for'):
            ...
            elif os.path.exists(getattr(args, 'like')) and os.path.isfile(getattr(args, 'like')):
                search_using_an_image(args)
            else:
                print('--like takes path of an image or a folder containing images')
        else:
            print('search command requires one of the following: --query, --for, --like')
