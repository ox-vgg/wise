...
cat << EOF > ${SEARCH_RESULT_GND_FN}
query_id,query_text,media_type,rank,filename,start_time,end_time,score
0,"cooking",video,0,"frying-vegetables/mT7vy1-KP_Q_000398_000408.mp4",0.5,9.5,0.354
0,"cooking",video,1,"frying-vegetables/mwkOrWZxvrU_000006_000016.mp4",0.0,1.5,0.349
0,"cooking",video,2,"frying-vegetables/5E20wCGF6Ig_000122_000132.mp4",9.5,9.5,0.336
...
0-1,"cooking and music",video and audio,0,"frying-vegetables/5E20wCGF6Ig_000122_000132.mp4",0.0,9.5,0.479
EOF
...
if [ ! -d "${SEARCH_RESULT_CSV_FN}" ]; then
    ...
    python search.py \
           ...
           --project-dir $KINETICS_PROJECT_DIR
    if ! cmp -s ${SEARCH_RESULT_GND_FN} ${SEARCH_RESULT_CSV_FN}; then
        echo "Search results are unexpected"
        diff "${SEARCH_RESULT_CSV_FN}" "${SEARCH_RESULT_GND_FN}"
    else
        echo "Search passed"
    fi
fi
