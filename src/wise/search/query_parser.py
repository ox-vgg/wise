    ...
    while index < len(cmd):
        ...
        if not query_extract_done:
            ...
            if cmd[index] in QUERY_QUOTE_CHAR:
                ...
                while next_char_index < len(cmd):
                    ...
                    if next_space == -1:
                        message = (
                            f"expected a space character after index {index}"
                        )
                        break
                    ...
                    if cmd[next_char_index] in QUERY_EMBEDDING_VEC_OPS:
                        ...
                        break
                    target_link_index = which_keyword_is_next(
                        cmd, next_char_index, SEARCH_TARGET_LINK
                    )
            ...
            parsed_queries["query"][query_index]["query_type"].append(
                query_type
            )
        ...
        if query_extract_done:
            ...
            if cmd[index] in QUERY_EMBEDDING_VEC_OPS:
                ...
                if cmd[index + 1] == " ":
                    parsed_queries["query"][query_index][
                        "query_embedding_vec_op"
                    ].append(cmd[index])
                    index += 2
                    query_extract_done = False
            ...
            # Step 2.3 parse search target (e.g. audio, video, metadata)
            target_index = which_keyword_is_next(
                cmd, index, SEARCH_TARGET_LIST
            )
            if target_index == -1:
            ...
            # Step 2.4 parse sub-query result merge operator (e.g. AND, OR)
            index += 1
            merge_op_index = which_keyword_is_next(
                cmd, index, QUERY_RESULT_MERGE_OPS
            )
            if merge_op_index == -1:
            ...
            query_extract_done = False
            parsed_queries["query"].append(
                copy.deepcopy(query_spec_template)
            )  # space for new sub-queries
            query_index = query_index + 1

    query_parser_status = {"status": status, "message": message}
