...
class FeatureSearchIndex(SearchIndex):
    ...
    def get_index_filename(self, index_type):
        return self.index_dir / (
            self.modality_type.value + "-" + index_type + ".faiss"
        )
    ...
    def create_index(self, index_type, overwrite=False):
        ...
        if index_type == "IndexIVFFlat":
            ...
            index = faiss.IndexIVFFlat(
                quantizer, feature_dim, cell_count, faiss.METRIC_INNER_PRODUCT
            )
            index.set_direct_map_type(
                faiss.DirectMap.Hashtable
            )  # Hashtable needed to support non-sequential ids
            ...
            shuffled_features.enable_read(
                shard_shuffle=True, shuffle_values=True
            )

            train_features = np.ndarray(
                (train_count, feature_dim), dtype=np.float32
            )
    ...
    def load_index(self, index_type):
        ...
        self.index = faiss.read_index(
            index_fn.as_posix(), faiss.IO_FLAG_READ_ONLY
        )
        return True
    ...
    def is_internal_search_supported(self) -> bool:
        ...
        if self.index_type == "IndexFlatIP":
            ...
            return (
                ...
                and not self.index.direct_map.no()
            )
