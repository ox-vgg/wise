...
class SqliteSearchIndex(SearchIndex):
    def __init__(self, modality_type, asset_id, asset):
        ...
        assert (
            asset["metadata_db_type"] == "sqlite"
        ), "SqliteSearchIndex only supports the SQLite database engine"
    ...
    def create_index(self, index_type, overwrite=False):
        index_type = (
            "FTS5"  # use full text search version 5 of sqlite by default
        )
        self.metadata_table_fts = f"{self.metadata_table}_fts"
        if (
            self.sqlite_table_exists(self.metadata_db, self.metadata_table_fts)
            and not overwrite
        ):
            logger.warning(
        ...
        with sqlite3.connect(self.metadata_db) as sqlite_connection:
            ...
            fts_data = []
            for row in cursor.execute(
                f"SELECT {metadata_colnames_csv} FROM {self.metadata_table}"
            ):
                fts_data.append(row)
    ...
    def load_index(self, index_type):
        exists = self.sqlite_table_exists(
            self.metadata_db, self.metadata_table_fts
        )

        if self.sqlite_table_exists(self.metadata_db, self.metadata_table_fts):
            with sqlite3.connect(
                "file:" + str(self.metadata_db) + "?mode=ro", uri=True
    ...
    def search(self, modality_type, query, topk=5, query_type="text"):
        ...
        ), "SqliteSearchIndex only supports metadata search"
