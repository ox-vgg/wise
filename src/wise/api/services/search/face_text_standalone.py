...
def describe_face_text_query(
    ...
    *,
    ...
    supports_text = (
        extractor is not None and extractor.extract_text_features is not None
    )
...
def resolve_face_text_embedder(
    ...
    *,
    ...
    has_text_queries, has_image_queries, supports_text = (
        describe_face_text_query(
            q,
...
async def run_face_text_search_standalone(
    ...
    *,
    ...
    async def _search_text(
        features, start_idx, end_idx, _filter_specs, vector_id_constraint=None
    ):
        return search_service.search_with_feature(
            features,
            media_type=media_type,
    ...
    def _resolve_text_vector_ids(face_search_output):
        if media_type == MediaType.VIDEO:
            ...
            return []
        face_media_ids = [
            meta.media_id for meta in face_search_output.metadata
        ]
    ...
    def _build_text_ranked_keys(text_search_output):
        return [
            (meta.media_id, meta.timestamp)
            for meta in text_search_output.metadata
        ]
