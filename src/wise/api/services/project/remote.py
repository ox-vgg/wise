...
class RemoteWiseProjectService(WiseProjectService):
    ...
    def info(self) -> ProjectInfo:
        ...
        if not data:
            raise ValueError(
                "Failed to retrieve project info from remote service"
            )
    ...
    async def forward(self, full_path: str, request: Request):
        ...
        url = httpx.URL(
            path=full_path, query=request.url.query.encode("utf-8")
        )
    ...
    async def featured(
        ...
        if not data:
            raise ValueError(
                "Failed to retrieve featured items from remote service"
            )
    ...
    async def search(
        ...
        url = httpx.URL(path=endpoint, query=request.url.query.encode("utf-8"))
        rp_req = self.client.build_request(
            request.method,
            url,
            headers=request.headers.raw,
        ...
        if not data:
            raise ValueError(
                "Failed to retrieve search results from remote service"
            )
        response = common.SearchResponse.model_validate(data)
        response = self.modify_response(response)
    ...
    def modify_vector_info(
        self, vectors: list[common.VectorInfo]
    ) -> list[common.VectorInfo]:
        for item in vectors:
    ...
    def modify_image_results(
        self, results: common.ImageResults
    ) -> common.ImageResults:
        images = {}
    ...
    def modify_video_results(
        self, results: common.VideoResults | common.VideoAudioResults
    ) -> common.VideoResults | common.VideoAudioResults:
        videos = {}
        for item in results.videos.values():
            item.id = f"{self.name}/{item.id}"
            item.timeline_hover_thumbnails = (
                f"shard/{self.name}/{item.timeline_hover_thumbnails}"
            )
            videos[item.id] = item
        results.videos = videos

        results.unmerged_windows = self.modify_vector_info(
            results.unmerged_windows
        )
        results.merged_windows = self.modify_vector_info(
            results.merged_windows
        )
        return results

    def modify_response(
        self, response: common.SearchResponse
    ) -> common.SearchResponse:
        # Modify the response to adjust thumbnail URLs
        if response.image_results is not None:
            response.image_results = self.modify_image_results(
                response.image_results
            )

        if response.video_results is not None:
            response.video_results = self.modify_video_results(
                response.video_results
            )

        if response.video_audio_results is not None:
            response.video_audio_results = self.modify_video_results(
                response.video_audio_results
            )
    ...
    def get_thumbnail_reader(self):
        ...
        def inner(
            ...
            thumbs = list(map(_thumbnail_url, vector_and_media_metadata_list))
            return thumbs
    ...
    @classmethod
    def get_info(
        cls, projects: dict[str, "RemoteWiseProjectService"], name: str
    ):
