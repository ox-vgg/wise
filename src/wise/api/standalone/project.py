...
def get_media_file(
    ...
    try:
        ...
        if config.redirect_media_url_by_path:
            num_components = min(
                config.redirect_media_url_num_components,
                len(file_path.parts) - 1,
            )
            parts = (config.redirect_media_url_prefix,) + file_path.parts[
                -num_components:
            ]
        ...
        if metadata.media_type in {
            ...
            content_type = (
                f"{metadata.media_type.value}/{metadata.format}"
            ...
            return StreamingResponse(
                send_bytes_range_requests(
                    open(file_path, mode="rb"), start, end
                ),
                headers=headers,
            ...
            if metadata.source_collection.type is SourceCollectionType.DIR:
                ...
                if file_path.is_file():
                    return FileResponse(
                        file_path,
                        media_type=f"image/{metadata.format.lower()}",
                    )
...
def get_thumbnail(
    ...
    try:
        if high_res:
            thumbnail = project_service.thumbnail(
                media_id, timestamp, highres=True
            )
        else:
            try:
                thumbnail = project_service.thumbnail(
                    media_id=media_id, timestamp=timestamp
                )
            except ThumbnailNotFoundException as e:
                if config.use_shots:
                    # If no thumbnail found, try to get a high-res thumbnail from the original video
                    thumbnail = project_service.thumbnail(
                        media_id=media_id, timestamp=timestamp, highres=True
                    )
                else:
                    raise e
        return Response(
...
def get_storyboard_image(
    ...
    num_seconds_per_image = (
        2 if project_info.num_thumbnails < (2 * 30 * 60) else 4
    )
    try:
...
@router.get(
    ...
    response_model_exclude=set(
        ["id", "source_collection_id", "size_in_bytes", "date_modified"]
    ),
...
def get_metadata(_id: int, project_service: ProjectServiceDep):
    try:
        ...
        return MediaMetadata(
            **media_metadata.model_dump(exclude={"source_collection"})
        )
    except MediaNotFoundException:
...
def _get_facets_with_previews(project_service: LocalWiseProjectService):
    with project_service.wise_project.db_engine.connect() as conn:
        from wise.db.tables.facets import (
        ...
        for f in facets:
            ...
            if clusters:
                ...
                assignments = conn.execute(
                    sa.select(facet_metadata_table).where(
                        facet_metadata_table.c.cluster_id.in_(cluster_ids)
                    )
                ).fetchall()
                ...
                vector_info = {}
                if all_vector_ids:
                    metadata_list = project_service.wise_project.get_vector_media_metadata_for_ids(
                        all_vector_ids
                    )
                    ext_metadata_list = project_service.wise_project.get_vector_ext_metadata_for_ids(
                        f.feature_extractor_id, all_vector_ids
                    )
                    for m, ext in zip(metadata_list, ext_metadata_list):
                        area = (
                            (ext.bbox.w * m.width) * (ext.bbox.h * m.height)
                            if hasattr(ext, "bbox")
                            else 0
                        )
                        vector_info[m.id] = {
                ...
                for c_id in cluster_ids:
                    ...
                    vids_info = [
                        vector_info[vid] for vid in vids if vid in vector_info
                    ]
                    if vids_info:
...
def get_facets_index(
    ...
    if not config.enable_facets:
        raise HTTPException(
            status_code=404, detail="Facets feature is disabled."
        )
    if not isinstance(project_service, LocalWiseProjectService):
        return HTMLResponse(
            "<h1>Facets Unavailable</h1><p>The Facets interface is not currently supported while WISE is running in Aggregator (Multi-Shard) Mode. Please run WISE in Standalone mode to access this feature.</p>",
            status_code=400,
        )

    facets_html_path = (
        config.project_dir.parent.parent.parent
        / "frontend"
        / "dist"
        / "facets.html"
    )
    ...
    try:
        ...
        return HTMLResponse(
            "Facets UI not built. Please run npm run build in frontend.",
            status_code=500,
        )
...
@router.get(
    "/facets/{facet_name}/{feature_extractor_slug:path}/cluster/{cluster_id}/",
    response_class=HTMLResponse,
)
def get_facets_cluster_detail(
    config: ConfigDep,
    project_info: ProjectInfoDep,
    project_service: ProjectServiceDep,
    facet_name: str,
    feature_extractor_slug: str,
    cluster_id: int,
):
    if not config.enable_facets:
        raise HTTPException(
    ...
    with project_service.wise_project.db_engine.connect() as conn:
        ...
        facet = conn.execute(
            sa.select(facets_table).where(
                ...
                facets_table.c.feature_extractor_id.contains(
                    feature_extractor_slug
                ),
            )
        ).first()
        ...
        cluster_size = conn.execute(
            ...
            .select_from(facet_metadata_table)
            .where(
                facet_metadata_table.c.cluster_id == cluster_info.cluster_id
            )
        ).scalar()
...
@router.get(
    "/facets/{facet_name}/{feature_extractor_slug:path}/",
    response_class=HTMLResponse,
)
def get_facets_cluster_overview(
    config: ConfigDep,
    project_info: ProjectInfoDep,
    project_service: ProjectServiceDep,
    facet_name: str,
    ...
    if not isinstance(project_service, LocalWiseProjectService):
        raise HTTPException(
            status_code=400, detail="Facets only supported on local projects"
        )
    ...
    with project_service.wise_project.db_engine.connect() as conn:
        from wise.db.tables.facets import cluster_metadata_table, facets_table
        ...
        total_clusters = conn.execute(
            sa.select(sa.func.count())
            .select_from(cluster_metadata_table)
            .where(cluster_metadata_table.c.facet_id == facet.id)
        ).scalar()
...
@router.get(
    "/api/facets/{facet_name}/{feature_extractor_slug:path}/cluster/{cluster_id}/info"
)
def get_facet_cluster_info_api(
    facet_name: str,
    feature_extractor_slug: str,
    cluster_id: int,
    project_service: ProjectServiceDep,
    ...
    if not isinstance(project_service, LocalWiseProjectService):
        return JSONResponse(
            {"error": "Facets are not supported in Aggregator Mode."},
            status_code=400,
        )
    with project_service.wise_project.db_engine.connect() as conn:
        from wise.db.tables.facets import (
...
def get_published_clusters(
    ...
    with project_service.wise_project.db_engine.connect() as conn:
        ...
        query = (
            sa.select(
                ...
                sa.func.count(sa.distinct(vectors_table.c.media_id)).label(
                    "unique_media_count"
                ),
            )
            .select_from(
                cluster_metadata_table.outerjoin(
                    facet_metadata_table,
                    cluster_metadata_table.c.cluster_id
                    == facet_metadata_table.c.cluster_id,
                ).outerjoin(
                    vectors_table,
        ...
        cluster_stats = {
            c.cluster_id: {
                "size": c.size,
                "unique_media_count": c.unique_media_count,
    ...
    all_vector_ids = [
        vid for vids in cluster_to_vectors.values() for vid in vids
    ]

    vector_info = {}
    if all_vector_ids:
        metadata_list = (
            project_service.wise_project.get_vector_media_metadata_for_ids(
                all_vector_ids
            )
        )
        with project_service.wise_project.db_engine.connect() as conn:
            ...
            facet = conn.execute(
                sa.select(facets_table).where(facets_table.c.id == facet_id)
            ).first()
        ext_metadata_list = (
            project_service.wise_project.get_vector_ext_metadata_for_ids(
                facet.feature_extractor_id, all_vector_ids
            )
        )
...
def get_facets_cluster_faces(
    ...
    with project_service.wise_project.db_engine.connect() as conn:
        ...
        facet = conn.execute(
            sa.select(facets_table).where(
                facets_table.c.id == cluster_row.facet_id
            )
        ).first()

    # Get metadata from internal.db
    metadata = project_service.wise_project.get_vector_media_metadata_for_ids(
        vector_ids
    )
    ext_metadata = (
        project_service.wise_project.get_vector_ext_metadata_for_ids(
            facet.feature_extractor_id, vector_ids
        )
    )
...
async def get_published_cluster_faces_by_media(
    cluster_id: int, request: Request, project_service: ProjectServiceDep
):
    ...
    with project_service.wise_project.db_engine.connect() as conn:
        ...
        assignments = conn.execute(
            sa.select(facet_metadata_table.c.vector_id).where(
                facet_metadata_table.c.cluster_id == cluster_id
            )
        ).fetchall()
    ...
    for i in range(0, len(vector_ids), chunk_size):
        ...
        ext_metadata.extend(
            project_service.wise_project.get_vector_ext_metadata_for_ids(
                facet.feature_extractor_id, chunk
            )
        )
