...
feature_extractor = FeatureExtractorFactory(
    args.feature_extractor, default_config
)

image_tensor = torch.randn(
    args.batch_size, 3, 1008, 1008
...
with torch.inference_mode():
    ...
    if feature_extractor.preprocess_audio is not None:
        preprocessed_audio = torch.cat(
            [
                feature_extractor.preprocess_audio(x.unsqueeze(0))
...
if args.export:
    ...
    visual_inputs = (
        (preprocessed_image,) if preprocessed_image is not None else tuple()
    )
    audio_inputs = (
        (preprocessed_audio.squeeze(1),)
        if preprocessed_audio is not None
        else tuple()
    )
    text_inputs = (preprocessed_text["input_ids"],)
    text_inputs += (
        (preprocessed_text["attention_mask"],)
        if "attention_mask" in preprocessed_text
        else tuple()
    )
...
def _run(session: ort.InferenceSession, inputs):
    ort_inputs = {
        name: value.detach().numpy() for name, value in inputs.items()
    }
...
def _verify_outputs(ort_outputs, outputs):
    ...
    for ort_output, (k, model_output) in zip(ort_outputs, outputs.items()):
        ...
        logger.info(
            f"({k}) diff - mean: {diff.mean()}, min: {diff.min()}, max: {diff.max()}"
        )
...
if preprocessed_image is not None:
    ...
    if (
        ...
        orig_sizes = [
            (image.shape[2], image.shape[1]) for image in image_tensor
        ]  # list of (width, height) tuples
        # Preprocess image (including resizing)
        images = feature_extractor.processor(
    ...
    if (
        ...
        scores, embeddings, boxes = sort_by_objectness(
            scores_tensor, embeddings_tensor, boxes_tensor
        )
        ...
        assert np.allclose(
            a.vectors, b.vectors, rtol=1e-5, atol=1e-3
        ), f"Feature vectors do not match"
        assert len(a.metadata) == len(
            b.metadata
        ), f"Metadata length mismatch: {len(a.metadata)} != {len(b.metadata)}"
        ...
        for c, d in zip(a.metadata, b.metadata):
            ...
            assert np.allclose(
                x_arr, y_arr, rtol=1e-5, atol=1e-3
            ), f"Metadata mismatch: {c} != {d}"
        ...
        _verify_outputs(
            ort_outputs,
            {
                "embeddings": np.concatenate(
                    [x.vectors for x in image_features]
                ),
            },
        )
