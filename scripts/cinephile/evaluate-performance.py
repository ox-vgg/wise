...
def evaluate_performance(wise_url, queries, strategy_func, verbose=False):
    ...
    Returns the overall recall. If verbose is True, prints detailed logs.
    """
    ...
    for i, item in enumerate(queries):
        ...
        if 'video_results' in search_results and search_results['video_results'] and 'merged_windows' in search_results['video_results']:
            ...
            for window in search_results['video_results']['merged_windows']:
                ...
                if media_id and media_id in videos_map:
                    ...
                    if filename:
                        retrieved_files.add(os.path.basename(filename))

        correct_retrieved = [f for f in retrieved_files if f in expected_files]
        
        num_correct = len(correct_retrieved)
        ...
        if verbose:
            ...
            if num_correct > 0:
                print(f"  Found correct files: {correct_retrieved}")
    ...
    recall = overall_correct / overall_expected if overall_expected > 0 else 0
    return recall
